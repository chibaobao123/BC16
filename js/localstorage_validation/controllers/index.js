...
function renderTableSinhVien(arrSV) {// input [sv,sv,{maSinhVien:'',tenSinhVien:''},...]
    var stringHTML = '';
...
document.querySelector('#btnTimKiem').onclick = function () {
    ...
    for (var index = 0; index < mangSinhVien.length; index++) {
        ...
        var sinhVien = mangSinhVien[index];
        if (sinhVien.tenSinhVien.toLowerCase().trim().search(tuKhoa) !== -1 || sinhVien.soDienThoai.trim().search(tuKhoa) !== -1) {
